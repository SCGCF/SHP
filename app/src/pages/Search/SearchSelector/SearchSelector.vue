  ...
  <div class="clearfix selector">
    <div class="type-wrap logo">
      ...
      <div class="value logos">
        <ul class="logo-list" @click="sendTrademark">
          <li v-for="trademark in trademarkList" :key="trademark.tmId" :data-tmName="`${trademark.tmId}:${trademark.tmName}`" >{{trademark.tmName}}</li>
          <!-- <li>TCL</li>
          <li>长虹（CHANGHONG）</li>
          <li>飞利浦（PHILIPS）</li>
          <li>风行电视</li>
    ...
    <div class="type-wrap" v-for="attrs in attrsList" :key="attrs.attrId">
      ...
      <div class="fl value">
        <ul class="type-list">
          <li v-for="(attrValue,index) in attrs.attrValueList" :key="index"  @click="sendAttrs(attrs, attrValue)">
            <a>{{attrValue}}</a>
          </li>
...
<script>
  export default {
    ...
    methods: {
      sendTrademark(e) {
        let {tmname} = e.target.dataset
        if(tmname) {
          this.$emit("getTrademark", tmname)
        }
      },
      sendAttrs(attrs, attrValue) {
        this.$emit("getAttrs", `${attrs.attrId}:${attrValue}:${attrs.attrName}`)
      }
    },  
  }
</script>
