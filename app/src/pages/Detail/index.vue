            ...
            <div class="priceArea">
              ...
              <div class="priceArea2">
                ...
                <div class="fixWidth">
                  ...
                  <em class="t-gray">{{skuInfo.skuDesc}} </em>
                </div>
              </div>
            </div>
            <div class="support">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spuSale, index) in spuSaleAttrList" :key="index" >
                ...
                <dd changepirce="0" @click="change(spuSale.spuSaleAttrValueList, spuSaleAttrValue)" :class="`${spuSaleAttrValue.isChecked == 0 ? 'active': ''}`" v-for="(spuSaleAttrValue, index) in spuSale.spuSaleAttrValueList" :key="index">{{spuSaleAttrValue.saleAttrName}}</dd>
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="skuNum" @change="updateNum">
...
<script>
  ...
  export default {
    ...
    methods: {
      async addShopCar() {
        try{
            let result = await this.$store.dispatch("addShopCar", {skuNum: this.skuNum, skuId: this.$route.params.kusId})
            //信息详情存储到本地会话中
            // 在跳转到购物车模块进行展示
      ...
      change(arr, objVal) {
        arr.forEach(element => {element.isChecked = 1})
        objVal.isChecked = 0
      }
    },
    computed: {
    ...
    mounted() {
      this.$store.dispatch("getGoodsSimple", this.$route.params.kusId)
    },
  }
</script>
